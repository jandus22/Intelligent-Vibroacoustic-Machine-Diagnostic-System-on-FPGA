...
class UdpReader(QThread):
    ...
    def _handle_measurement(
        ...
        if not frame.measurement_emitted:
            ...
            self.line_received.emit(
                ...
                f"elements={len(frame.values)}, channel={frame.channel}"
            )
    ...
    def _handle_board_status(
        ...
        self.line_received.emit(
            "RX STATUS: "
            f"board={STATE_NAMES.get(status.board_state, status.board_state)}, "
            f"dma={STATE_NAMES.get(status.dma_state, status.dma_state)}, "
            f"model={STATE_NAMES.get(status.model_state, status.model_state)}, "
            f"ethernet={STATE_NAMES.get(status.ethernet_state, status.ethernet_state)}, "
...
class MainWindow(QWidget):
    ...
    def prepare_fft(
        ...
        if data_kind == DATA_TIME_SAMPLES:
            ...
            # Obecny generator 0..1023 jest traktowany jak próbki czasowe.
            return np.abs(np.fft.rfft(values))
    ...
    def on_measurement(
        ...
        self.label_rate.setText(
            f"DATA RATE: {self._rate_hz:.1f} Hz"
        )
    ...
    def on_board_status(self, status: BoardStatus):
        ...
        if status.last_error != 0:
            self.log.appendPlainText(
                f"BOARD ERROR: code={status.last_error}"
            )
