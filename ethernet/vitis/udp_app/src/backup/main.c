...
/*
 * Dane znajdujące się za trzema słowami nagłówka DMA są obecnie traktowane
 * jako moduł FFT. Jeśli przed DMA pojawią się surowe próbki czasowe, należy
 * zmienić tę wartość na VIBR_DATA_TIME_SAMPLES.
 */
...
)
{
    ...
    #define TEST_CLASS_HOLD_FRAMES  180u
    selected_class = (uint8_t)((frame_id / TEST_CLASS_HOLD_FRAMES) % 3u);
...
u32_t get_time_ms(void)
{
    ...
    return (u32_t)(
        (current_time * 1000ULL) / COUNTS_PER_SECOND
    );
}
...
int main(void)
{
...

    {
        ...
        uint32_t last_status_ms = get_time_ms();
        ...
        while (1) {
            ...
            frame_timestamp_us = (uint64_t)get_time_ms() * 1000ULL;

            /* 0x01: cztery pakiety danych FFT z tym samym frame_id. */
